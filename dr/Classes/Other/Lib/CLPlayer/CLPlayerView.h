...
typedef NS_ENUM(NSUInteger, HiddenType) {
    HiddenNever = 0, ///<小屏和全屏都不隐藏
    HiddenAlways,    ///<小屏和全屏都隐藏
    HiddenSmall,     ///<小屏隐藏，全屏不隐藏
    HiddenFull,     ///<小屏不隐藏，全屏隐藏
};
...
typedef void(^FullChangeBlcok)(void);
...
/**退出按钮隐藏方式，默认不隐藏*/
@property (nonatomic, assign) HiddenType    exitButtonHiddenType;
/**拉伸方式，默认全屏填充*/
@property (nonatomic, assign) VideoFillMode           videoFillMode;
/**顶部工具条隐藏方式，默认不隐藏*/
@property (nonatomic, assign) HiddenType    topToolBarHiddenType;
/**全屏状态栏隐藏方式，默认不隐藏*/
