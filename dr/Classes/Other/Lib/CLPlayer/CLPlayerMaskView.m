...
- (void)makeConstraints{
    ...
    [self.exitButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(statusBarH + navBarH);
    ...
    CGFloat viewWH = 26;
    CGFloat viewPadding = (ToolBarHeight - viewWH) / 2;
    //返回按钮
    [self.backButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(viewPadding);
        make.top.mas_equalTo(viewPadding);
        make.size.mas_equalTo(CGSizeMake(viewWH, viewWH));
    }];
    //播放按钮
...
- (UIButton *) backButton{
    if (_backButton == nil){
        ...
        [_backButton setBackgroundImage:[self getPictureWithName:@"CLBackBtn"] forState:UIControlStateNormal];
        [_backButton setBackgroundImage:[self getPictureWithName:@"CLBackBtn"] forState:UIControlStateHighlighted];
...
- (UIButton *) playButton{
    if (_playButton == nil){
        ...
        [_playButton setBackgroundImage:[self getPictureWithName:@"CLPauseBtn"] forState:UIControlStateSelected];
...
- (UIButton *) fullButton{
    if (_fullButton == nil){
        ...
        [_fullButton setBackgroundImage:[self getPictureWithName:@"CLMaxBtn"] forState:UIControlStateNormal];
        [_fullButton setBackgroundImage:[self getPictureWithName:@"CLMinBtn"] forState:UIControlStateSelected];
...
- (UIButton *)muteButton
{
    if (_muteButton == nil) {
//        _failButton        = [[UIButton alloc] init];
//        [_muteButton setBackgroundImage:<#(nullable UIImage *)#> forState:<#(UIControlState)#>];
//        [_muteButton addTarget:self action:@selector(failButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _muteButton;
}
...
- (UIButton *)exitButton
{
    if (_exitButton == nil) {
        ...
        [_exitButton setTitle:@"退出视频" forState:UIControlStateNormal];
        [_exitButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        _exitButton.titleLabel.font = [UIFont systemFontOfSize:13.0];
        _exitButton.backgroundColor = [UIColor colorWithWhite:0.5 alpha:0.5];
        _exitButton.layer.masksToBounds = YES;
        _exitButton.layer.cornerRadius = 12;
...
- (void)exitButtonAction:(UIButton *)button{
    if (_delegate && [_delegate respondsToSelector:@selector(cl_exitButtonAction:)]) {
        [_delegate cl_exitButtonAction:button];
    }else{
        NSLog(@"没有实现代理或者没有设置代理人");
    }
}
