...
- (void)updateFrames
{
    ...
    if (IsBangIPhone) {
        self.indexLabel.xl_centerY = 44 + 15;
    }else
    {
        self.indexLabel.xl_centerY = 20 + 15;
    }
...
- (void)show
{
    ...
    if (self.datasource && [self.datasource respondsToSelector:@selector(showPhotoBrowser:)]) {
        [self.datasource showPhotoBrowser:self];
    }
}
...
- (void)dismiss
{
    ...
    [UIView animateWithDuration:XLPhotoBrowserHideImageAnimationDuration animations:^{
        ...
        if (self.datasource && [self.datasource respondsToSelector:@selector(hiddenPhotoBrowser:)]) {
            [self.datasource hiddenPhotoBrowser:self];
        }
    }];
}
