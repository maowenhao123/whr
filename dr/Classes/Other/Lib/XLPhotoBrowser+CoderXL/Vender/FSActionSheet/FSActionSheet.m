...
- (CGFloat)currentScreenWidth {
    ...
    CGFloat screenWidth_ = fmin(screenSize.width, screenSize.height);  // 该值为屏幕竖屏下的屏幕宽度
    CGFloat screenHeight_ = fmax(screenSize.width, screenSize.height); // 该值为屏幕竖屏下的屏幕高度
    // 判断屏幕方向
    UIInterfaceOrientation orientation = [_popupVC preferredInterfaceOrientationForPresentation];
    // 横屏
    if (orientation == UIInterfaceOrientationLandscapeLeft  || orientation == UIInterfaceOrientationLandscapeRight) {
        currentScreenWidth = screenHeight_;
    } else {
        currentScreenWidth = screenWidth_;
    }
...
- (CGFloat)currentScreenHeight {
    ...
    // 横屏
    if (orientation == UIInterfaceOrientationLandscapeLeft  || orientation == UIInterfaceOrientationLandscapeRight) {
        currentScreenHeight = screenWidth_;
    } else {
        currentScreenHeight = screenHeight_;
    }
...
- (UITableView *)tableView {
    ...
    [_tableView setEstimatedSectionHeaderHeightAndFooterHeight];
